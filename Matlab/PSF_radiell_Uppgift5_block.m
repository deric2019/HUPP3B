...
D_block= 0.15; % Blockdiamater på linsen
...
c_4 = 4.8230e4; % för korrektion av linsens fasmodulering, se uppgift 2 (i uppgift 1 ska c_4=0) 
...
if sampla_om_till_2D==1
    ...
    samplavst_2D = (400*3)/(4999*250e3); % samplingsavståndet hos omsamplade PSFen: ska vara lika med samplingsavståndet hos den perfekta bilden i detektorplanet
end
...
T_lins = exp(1i*fi_lins).*((D_block/2<=rmat) & (rmat<=D_lins/2)); % linsens transmissionsfunktion
...
for berpunkt_nummer=1:length(u_eval_vekt) % gå igenom beräkningspunkterna längs u-axeln
    ...
    E_PSF_i_aktuellt_u = sum(sum((T_lins.*exp(1i*k.*r )./r)*a)*a); % fältet i beräkningspunkten, beräknat med HFM-integralen
...
I_perf=double(imread('highly_resolved_photo_of_Marks_surroundings.jpg')); figure
I=ifft2c(fft2c(I_perf).*fft2c(PSF_2D));
figure(222)
imagesc(I)
colormap(gray)
