...
figure('Name','Djupskillnad','NumberTitle','off' )
r1 = linspace(-D_lins/2,D_lins/2,1000);
phi = c_4*r1.^4;
z = 1/(2*k)*phi;
z2=r1.^2/(4*f_lins);
plot(r1,z,'r');
title('Djupskillnad i förhallande till radie')
xlabel('Radie för spegeln (m)');
ylabel('Skillnad i djup (m)');
set(gca,'FontSize',14)

figure('Name','Spegelns totala djup', 'NumberTitle','off')
plot(r1,z2,'b')
title('Djupskillnad i förhallande till radie')
xlabel('Radie för spegeln (m)');
ylabel('Skillnad i djup (m)');
